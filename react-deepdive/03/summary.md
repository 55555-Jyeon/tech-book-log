...
이 함수의 시그니처는 useEffect와 동일하지만 실행 시점에 차이가 있습니다.
useEffect와 달리 useLayoutEffect는 모든 DOM의 변경 후에 동기적으로 발생합니다.
...
동기적으로 발생하는 특징으로 인해 리액트 컴포넌트는 useLayoutEffect가 완료될 때까지 기다리기 때문에 컴포넌트가 잠시 동안 일시 중지되는 것과 같은 일이 발생하게 됩니다. <br />
따라서 이러한 작동 방식으로 인해 웹 애플리케이션 성능에 문제가 발생할 수 있습니다.

###### Q. useLayoutEffect는 언제 사용하는게 좋을까?

성능에 문제가 생길 수 있으므로 반드시 필요한 경우에만 사용하는 것이 좋습니다.

예를 들어, DOM은 계산되었지만 이것이 화면에 반영되기 전에 하고 싶은 작업이 있을 때 사용하는 것이 좋습니다. <br />
특정 요소에 따라 DOM 요소를 기반으로 한 애니메이션이나 스크롤 위치 제어 등 화면에 반영되기 전에 하고 싶은 작업에 useEffect 대신 useLayoutEffect를 사용하면, 사용자가 페이지가 즉시 반응하는 것처럼 느낄 수 있습니다.
